...
git_sync() {
  ...
  ssh-agent bash -c "ssh-add /tmp/key.pem; git clone $PROJECT_CLONE_URL $PROJECT_CLONE_LOCATION"
  ...
  if [ "$IS_PULL_REQUEST" != false ]; then
    ssh-agent bash -c "ssh-add /tmp/key.pem; git fetch origin merge-requests/$PULL_REQUEST/head"
